...
class Ux implements UxDB {
  ...
  getUxDates = async (entryKey: string): Promise<any> => {
    const result = await UxModel.find({ "Perfanalytics.entryKey": entryKey })
  ...
  getUxByEntry = async (entryKey: string, date: string): Promise<any> => {
    const result = await UxModel.find({
      $and: [{ "Perfanalytics.entryKey": entryKey }, { "Perfanalytics.date": date }],
    })
      .sort({ "Perfanalytics.date": 1 })
      .limit(1);
  ...
  createUx = async (uxKey: string, document: object): Promise<any> => {
